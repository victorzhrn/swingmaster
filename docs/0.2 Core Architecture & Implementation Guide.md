...
- `AnalysisView.swift`: Presents swing metrics with an enhanced bottom navigation consisting of `KeyframeTabs` (jump to Ready/Back/Contact/Follow/End), `TimelineStripEnhanced`, and floating inline controls: `ViewModeControl` on the left and `CompareToggle` on the right. Accessible when processing is complete; displays simple metrics from SegmentMetrics (AI insights removed). Pro comparison uses `ProVideoLoader`, keeps trajectories per enabled type in sync, and loosely time‑syncs playback by offset.
