...
- Core
  ...
  - `VideoProcessor.swift`: Orchestrates end‑to‑end processing pipeline including pose extraction, object detection, contact analysis, swing detection, and metrics calculation for both live and file sources.
...
- Views
  ...
  - `AnalysisView.swift`: Presents swing metrics and enhanced navigation controls with `TimelineStripEnhanced`; only accessible when session.processingStatus is complete; displays simple metrics from SegmentMetrics (AI insights removed).
...
- `GeminiValidator`
  ...
  - Outputs: Validated swings with refined start/end frames, swing type, and confidence
...
- `VideoProcessor`
  ...
  - Guarantees: Orchestrates full pipeline off the main thread; batches work for files; incremental accumulation during live capture; exposes progress via `ProcessingState` (pose extraction → object detection → contact analysis → swing detection → metrics → validation)
