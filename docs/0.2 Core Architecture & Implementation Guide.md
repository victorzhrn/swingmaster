...
- Models
  ...
  - `Shot.swift`: Individual shot data. UI-agnostic by design (no `SwiftUI` imports or color mapping). Exposes `ShotType` with `shortLabel` and `accessibleName` for UI text and accessibility; any color mapping is provided via UI-layer extensions.
...
- Views
  ...
  - `AnalysisView.swift`: Presents swing metrics and enhanced navigation controls with `TimelineStripEnhanced`; only accessible when session.processingStatus is complete; displays simple metrics from SegmentMetrics (AI insights removed). Supports side‑by‑side pro comparison: loads a bundled pro `Shot` via `ProVideoLoader`, keeps trajectories per enabled type in sync, and time‑syncs playback with a simple offset (`pro.startTime + (userCurrent - user.startTime)`).
