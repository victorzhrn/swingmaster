...
- Processing: Pose extraction → Object detection (YOLO11) → Swing detection → Contact point analysis → Metrics and insights → AI validation
...
│   ├── ContactPointDetector.swift   # Ball-racket contact point analysis
│   ├── SwingDetector.swift          # Swing boundary detection
│   ├── MetricsCalculator.swift      # Kinematic metrics calculation
│   ├── GeminiValidator.swift        # Enhanced AI swing validation
...
- Core
  ...
  - `TennisObjectDetector.swift`: YOLO11-based detection of tennis balls and rackets in video frames; handles device orientation and coordinate transformation; provides confidence-based filtering. Supports on-demand file-based detection for a time window.
  - `ContactPointDetector.swift`: Analyzes ball and racket detection results to identify contact points; correlates timing with swing motion for impact analysis.
  ...
  - `GeminiValidator.swift`: Enhanced validation of `PotentialSwing`s with 30‑frame context, swing classification, and comprehensive analysis insights using pose and object detection data.
  - `VideoProcessor.swift`: Orchestrates end‑to‑end processing pipeline including pose extraction, object detection, contact analysis, swing detection, and AI validation for both live and file sources.
...
- Managers & Storage
  - `SessionStore.swift`: Enhanced with ProcessingStatus enum tracking (pending, extractingPoses, detectingObjects, calculatingMetrics, detectingSwings, validatingSwings, analyzingSwings, complete, failed); supports updateSession for reactive state changes; manages session metadata with retry counts and error tracking.
...
- Views
  ...
  - `AnalysisView.swift`: Presents comprehensive metrics, insights, score, and enhanced navigation controls with `TimelineStripEnhanced`; only accessible when session.processingStatus is complete; integrates on-demand object detection visualization, trajectory overlays, and contact point analysis.
...
- Utils
  ...
  - `TrajectoryComputer.swift`: On-demand trajectory computation (extraction, gap fill, smoothing) for joints/ball/racket used by the trajectory overlay.
...
- `ContactPointDetector`
  - Inputs: Ball and racket `ObjectDetection` sequences with timestamps
  - Outputs: Contact point events with timing and spatial coordinates
  - Guarantees: Temporal correlation of ball and racket proximity; handles detection gaps and false positives; provides confidence scoring
...
- `GeminiValidator`
  - Inputs: `PotentialSwing`s (≈30 frames), pose data, and object detection results
  - Outputs: Validated swings with refined start/end frames, swing type, confidence, contact analysis, and comprehensive insights
  - Guarantees: Uses 30‑frame context with pose and object data; network‑robust with retries and basic rate limiting; returns deterministic JSON schema with enhanced tennis-specific analysis
...
- `VideoProcessor`
  ...
  - Outputs: Collection of `AnalysisResult`s for detected swings with object detection integration
  - Guarantees: Orchestrates full pipeline off the main thread; batches work for files; incremental accumulation during live capture; exposes progress via `ProcessingState` (pose extraction → object detection → contact analysis → swing detection → metrics → validation → analysis)
...
- Live recording (Non-blocking MVP flow)
  ...
  5. `ProcessingManager` starts background processing:
     ...
     - `ContactPointDetector` analyzes contact points (status: analyzingContacts)
     ...
     - `GeminiValidator` validates swings with object data (status: validatingSwings x/y)
     - AI analysis with tennis-specific insights (status: analyzingSwings x/y)
  6. `VideoSessionCard` shows inline progress updates in real-time.
  7. On completion (status: complete), tapping card navigates to `AnalysisView` with object detection visualization.
...
- Video upload (Non-blocking MVP flow)
  ...
  5. `ProcessingManager` queues and processes video in background:
     ...
     - Maps VideoProcessor states (pose → object detection → contact analysis → swing detection → metrics → validation) to UI-friendly status messages
  6. `VideoSessionCard` displays real-time status updates inline with object detection progress.
  7. On completion, `ProcessingManager` saves enhanced results with object detection data via `AnalysisStore`.
  8. Tapping completed session (status: complete) navigates to `AnalysisView` with full object detection visualization and contact point analysis.
