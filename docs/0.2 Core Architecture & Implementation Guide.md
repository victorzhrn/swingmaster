...
│   ├── CameraManager.swift
│   ├── SessionStore.swift
│   └── AnalysisStore.swift   # NEW: MVP persistence of analysis-by-video
├── Mock/
...
- Storage layer
  ...
  - Guarantees (MVP): `SessionStore` saves session metadata (id/date/video file name/shot count). `AnalysisStore` saves per‑video analysis display data (`duration`, `[MockShot]`).
...
- Video upload
  ...
  4. Each detected swing is analyzed. On completion, `ContentView` maps results to `MockShot` and uses `AnalysisStore.save(videoURL:duration:shots:)`. `SessionStore` saves session metadata.
  5. Revisiting from `HistoryView` now attempts `AnalysisStore.load(videoURL:)`. If found, the app navigates directly to `AnalysisView` without reprocessing; otherwise it falls back to processing.
