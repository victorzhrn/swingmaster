...
├── Components/
│   ├── TimelineStrip.swift      // Accessible timeline band (56pt, VO adjustable)
│   └── ShotChipsRow.swift       // 44pt-high scrollable shot chips + prev/next
├── Storage/
│   ├── CoreDataManager.swift    // Database operations
│   ├── VideoStorage.swift       // File management
│   └── TennisApp.xcdatamodeld   // Core Data schema
├── Views/
│   ├── CameraView.swift         // Live recording (Start/End controls)
...
### 6. UI Components (Accessible)

#### TimelineStrip.swift
```swift
// 56pt glass timeline with snap-to-nearest markers and VoiceOver support
struct TimelineStrip: View {
    ...
    // Behavior:
    // - Maps shot times to X positions
    // - Snap-to-nearest on tap/drag (≥32pt hit area)
    // - VoiceOver: .adjustable (previous/next shot), descriptive labels/values
}
```

Key details:
- 56pt-high band using `.ultraThinMaterial` with 1pt stroke for contrast
- Markers use shape + label (FH/BH), not color alone; selected marker gets ring
- Haptics on selection; updates `currentTime` to the shot time

#### ShotChipsRow.swift
```swift
// 44pt-high horizontally scrollable chips as primary tap targets
struct ShotChipsRow: View {
    ...
    let onNext: () -> Void
    // Behavior:
    // - Each chip shows short label (FH/BH) + score
    // - Scrolls to keep selection centered
    // - Prev/Next chevrons for discrete navigation
}
```

Key details:
- Chips meet 44pt minimum target; strong contrast over video
- VoiceOver: each chip is a Button with full label (e.g., "Forehand, score 6.9")
- Redundant cues (text + shape/color) support color-blind users
...
### Live Recording Flow
```
1. User taps START → Begin recording clip (auto-stop at 10s)
2. AVCaptureVideoDataOutput → CMSampleBuffer
2. Convert to CVPixelBuffer
3. PoseProcessor.processFrame() → PoseFrame
4. SwingDetector.addFrame()
5. User taps END → Finalize last clip
6. On swing detection → Extract segment
7. MetricsCalculator → Generate metrics
8. Save to CoreData + VideoStorage
```
